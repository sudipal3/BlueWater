...
// Function to generate the additional procedure note for US guidance
// (Included to match the original script structure exactly; not triggered in Dental Block HTML)
function generateAdditionalProcedureNote() {
    ...
    additionalNote.innerHTML = `
        <h3>Procedure: Bedside ultrasound guidance for dental block</h3>
        <p><strong>Indication:</strong> ultrasound guidance for dental block placement for pain control and/or procedural anesthesia.</p>
        <p><strong>Procedure:</strong> limited ultrasound of the target anatomy to guide needle placement.</p>
        <p><strong>Interpretation:</strong> relevant structures were visualized dynamically during the procedure and the needle tip was identified in real time. Images were saved in the medical record and/or PACS as available.</p>
    `;
...
function removeMainHeader(outputArea) {
    // If there are no section divs and no additional note, clear header
...
function triggerMacro(buttonIds, macroButton) {
    ...
    buttonIds.forEach(id => {
        ...
        const section = id.substring(0, firstDashIndex);
        const buttonText = id.substring(firstDashIndex + 1);
        ...
        if (matchedButton) {
            const match = matchedButton.getAttribute('onclick').match(/'([^']+)'/);
            if (match) {
                const associatedText = match[1];
